...
function positionModelOnMarker(canvasX, canvasY) {
    ...
    // WICHTIG: Wenn der Marker wiedergefunden wird, den Ausblend-Timer ABBRECHEN!
    if (markerLostTimer !== null) {
        clearTimeout(markerLostTimer);
        markerLostTimer = null;
        // console.log("Ausblend-Timer abgebrochen: Marker wiedergefunden.");
    }

    const screenCoords = mapCanvasToScreen(canvasX, canvasY);
    // ... (restliche Positionierungsberechnung) ...

    const modelWidth = model.offsetWidth || 200; 
    const modelHeight = model.offsetHeight || 200;
...
function handleMarkerLost() {
    ...
    // Starte den Timer zum Ausblenden des Modells nach der definierten Verzögerung
    markerLostTimer = setTimeout(() => {
        if (model) {
            model.style.display = 'none';
            // console.log("Modell nach Verzögerung ausgeblendet.");
        }
        markerLostTimer = null; // Timer ist abgelaufen und kann für neue Aufrufe zurückgesetzt werden
    }, HIDE_DELAY_MS);
}
