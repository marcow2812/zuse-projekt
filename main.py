            ...
            for i, c in enumerate(corners):
                ...
                if overlay_path is not None:
                    ...
                    if overlay is None:
                        ...
                        warped = cv2.warpPerspective(overlay, M, (frame.shape[1], frame.shape[0]), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_TRANSPARENT)

                        # Alphamischung
                        if warped.shape[2] == 4:  # PNG mit Alpha
                            alpha = warped[:, :, 3] / 255.0
                            for cch in range(3):
                                frame[:, :, cch] = warped[:, :, cch] * alpha + frame[:, :, cch] * (1 - alpha)
                        else:
